let debug         = require('debug')('ringtail-deploy-client')
  , Q             = require('q')
...
function RingtailClient(options) {
  ...
  this.restartUrl    = 'http://' + this.serviceHost + ':8080/api/restart';
  this.preReqUrl     = 'http://' + this.serviceHost + ':8080/api/Prerequisite';
 ...
 * @return {promise}
 */ 
RingtailClient.prototype.restart = function restart(next) {
  let deferred = new Q.defer()    
    , restartUrl = this.restartUrl    
    ;

  request.get(restartUrl, function(err, response, body) {
    deferred.resolve(body);
  }); 
...
RingtailClient.prototype.installed = function installed(next) {
  let installedUrl = this.installedUrl    
    , requestTimeout = this.requestTimeout
    , deferred = Q.defer()    
    ;
